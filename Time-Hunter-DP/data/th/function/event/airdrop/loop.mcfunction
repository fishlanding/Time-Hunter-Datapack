# 计时
$execute if score Timer AirDropTimer matches 0 run scoreboard players set Timer AirDropTimer $(intervaltime)
execute if score Timer TimerSec matches 0 run scoreboard players remove Timer AirDropTimer 1
...
execute if score Timer AirDropTimer matches 0 run tellraw @a [{"translate": "game.ingame.tishiji","color": "light_purple"},{"translate": "game.event.airdrop.tellraw.2","color": "white"}]
# 选人
